...
- `2026-07-26` **서버 배포 설치본을 감독모드 전용으로 분리.** 사용자 요청에 따라 `1.0.33`부터 설치 패키지에서 `renewal_macro.exe`, `gamepad_test.exe`, 갱신 바로가기를 제외한다. 이전 통합 설치본을 덮어쓸 때 남은 갱신/테스트 파일과 바로가기도 `[InstallDelete]`로 제거해 설치 폴더에는 감독모드 본체·런처·필수 런타임만 유지한다.

- `2026-07-25` **잘못 열린 알림 패널 자동 복구 추가.** 일부 PC에서 해상도/좌표 차이로 하단 종 버튼이 눌려 오른쪽 `알림` 패널이 열린 뒤 자동화가 막히는 실측 화면을 반영했다. 매칭 루프가 0.25초마다 [오른쪽 20%의 긴 흰 패널 + 가운데 검은 오버레이 + 상단 검은 제목 띠]를 확인하고, 2회 연속 합의했을 때만 해상도 비율 좌표의 종 버튼을 다시 눌러 닫는다. 열린 동안 일반 타겟 매칭과 OCR 프레임 공개를 막아 추가 오클릭을 방지하며, 클릭 뒤 1.5초 재시도 제한으로 이전 프레임 때문에 다시 열리는 연타를 차단한다. 전체화면/Windows 제목 표시줄 모드를 모두 판정하고, 사용자 첨부 양성 화면은 1280×720~2560×1440 리사이즈 전부 감지, 저장된 정상 FC 화면 13장은 오탐 0. `compileall`/`pyflakes`/6개 단위 테스트 통과.
