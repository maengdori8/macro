"""Nuitka/PyInstaller 진입 스크립트 — **프로 빌드** (레포 루트).

일반 빌드(macro_main.py)와 이 파일의 차이는 딱 한 줄, 제품 식별자뿐이다.
진단 로직이 갈라지지 않도록 나머지는 같은 내용을 유지한다.
...
def _report_startup_failure(exc: BaseException) -> None:
    ...
    if log_dir:
        try:
            ...
            with open(log_path, "a", encoding="utf-8") as f:
                f.write(
                    f"[mAuto Pro 시작 실패] {datetime.datetime.now():%Y-%m-%d %H:%M:%S}\n"
    ...
    try:
        import ctypes
        ctypes.windll.user32.MessageBoxW(0, msg, "mAuto Pro 시작 오류", 0x10)  # MB_ICONERROR
    except Exception:
        pass
