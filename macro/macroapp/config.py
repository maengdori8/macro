...
EXIT_RETRY_HOLD_MAX_SECONDS = 10.5    # 절대 상한. 사용자 실측 '11초쯤에서 게임이 꺼짐'은
                                      # **안전선이 아니라 이미 위험 경계**다. 재개 호출과
                                      # 스케줄 지연으로 실제 정지는 nominal 보다 길어질 수
                                      # 있으므로 여유를 둔다. 한 번의 게임 종료 비용은
                                      # '그날 나머지 전부'라 시도 몇 번보다 훨씬 비싸다.
...
#: 열기 입력 뒤에도 안 뜨면 다시 누르는 간격(초).
#: ⚠️ 예전엔 5.0 이었다(0·5·10초에 딱 3번). 그런데 게임은 10초 얼어 있다가 막 깨어난
#: 참이라 **재개 직후 첫 START 가 자주 씹힌다** — 그러면 다음 기회가 5초 뒤라 종료가
#: 통째로 실패한다(실측: 재시도 4회 연속 실패). 사용자 지시대로 **확인 버튼이 보일
#: 때까지 짧은 간격으로 연타**한다.
#: 토글 걱정은 없다 — ConfirmSequence 는 그림이 **한 번이라도 보이면**(self.seen)
#: 다시 열지 않으므로, 열린 메뉴를 다음 입력이 도로 닫지 않는다.
EXIT_OPEN_RETRY_SECONDS = 0.4
#: 열기 입력 최대 횟수. 그림이 한 번이라도 보이면 더 누르지 않습니다.
#: 0.4초 간격으로 마무리 시한(EXIT_TIMEOUT_SECONDS=40초) 동안 계속 누를 수 있게 넉넉히.
EXIT_MAX_OPENS = 60
#: 인식 루프의 최소 간격(초).
...
#: 상대 닉네임 칸(스코어보드에서 스코어 오른쪽). **판 경계의 직접 증거**다 —
#: 닉네임이 바뀌면 의심의 여지 없이 새 판이다. 글자를 읽지 않고 픽셀 지문만 비교하므로
#: OCR 이 필요 없고 언어에 무관하다(사용자 제안, 2026-08-23).
#: 실측(녹화 12에피소드): 같은 상대 지문 거리 최대 24, 다른 상대 최소 60 — 임계 40으로 분리.
AUTO_EXIT_OPPONENT_REGION = (0.212, 0.088, 0.300, 0.122)
# ─── 종료 재시도 ───
