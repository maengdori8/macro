"""감독모드 즉시 종료 — 1회성 워커(정지 → 유지 → 재개 → 마무리).

형제 프로젝트(../mpause)의 같은 파일에서 이식했다. 판정과 방어 구조는 그대로이고
import 와 상수 이름만 이 앱에 맞췄다. 한쪽을 고치면 다른 쪽도 같이 고칠 것.
...
from macroapp import exit_core as core, ledger, winproc
from macroapp.config import (
    EXIT_HOLD_SECONDS as HOLD_SECONDS,
    EXIT_TARGET_PROCESS_NAME as TARGET_PROCESS_NAME,
    EXIT_TICK_SECONDS as TICK_SECONDS,
)
