...
version.txt / license.key / logs 가 있는 폴더를 정확히 찾습니다.
이 모듈 하나만 고치면 모든 호출부의 경로가 일관됩니다.

주의: Nuitka onefile에서 sys.executable은 '임시 압축해제 폴더'를 가리킵니다.
이 값을 쓰면 license.key와 로그가 매 실행마다 사라지는 임시 폴더에 저장되고,
런처는 version.txt를 못 찾아 0.0.0으로 읽습니다(mAuto에서 실제로 겪은 버그).
...
def app_dir() -> Path:
    ...
    # 소스 실행: mpauseapp/paths.py -> 상위(패키지) -> 상위(프로젝트 루트)
